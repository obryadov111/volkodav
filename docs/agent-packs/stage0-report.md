...
CI: добавлен `.github/workflows/agent-tests.yml` (тесты агента при изменениях в `agent/`); бэкендовый workflow не менялся — новые тесты в него попадают автоматически (`PACK_SIGNING_KEY` для тестов задаётся в `conftest.py`).
...
5. **Живая БД доступна с хоста не всегда** (соединение к опубликованному порту обрывалось), а `app_test_db` отставала по миграциям. Поэтому разработка велась в одноразовом контейнере Postgres, чтобы не задеть ни живую, ни общую тестовую БД.
