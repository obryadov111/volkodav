...
def upgrade() -> None:
    """Раньше это был no-op, помечавший вручную накатанный schema_fixed.sql
    как отправную точку. Теперь миграция сама создаёт базовую схему —
    отдельный SQL-файл больше не нужен, `alembic upgrade head` на пустой
    базе строит всё с нуля."""
    op.execute(SCHEMA_SQL)


def downgrade() -> None:
    """Базовая миграция вниз не откатывается — для этого нужно пересоздать БД."""
    pass
